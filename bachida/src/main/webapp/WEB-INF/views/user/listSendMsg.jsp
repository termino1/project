...
<script>
	$(function(){
		...
		$.each($list, function(i, message) {
			...
			$("<td></td>").text(message.receiver).appendTo($tr);
			var $content = $("<td></td>").text(message.messageContent).appendTo($tr);
			var $link = $("<a>").attr("href", "/bachida/user/sendMsgView/" + message.messageIdx);
		...
		if($pagination.prev>0) {
			...
			li.wrapInner($("<a></a").attr('href', '/bachida/user/sendMsgList?pageno='+ $pagination.prev))
		}
		for(var i=$pagination.startPage; i<=$pagination.endPage; i++) {
			li = $("<li></li>").text(i).appendTo(ul);
			if($pagination.pageno==i) 
				li.wrapInner($("<a></a").attr('href', '/bachida/user/sendMsgList?pageno='+ i).css({"background-color":"#337ab7", "border" : "1px solid #337ab7", "color":"white"}));
			else
				li.wrapInner($("<a></a").attr('href', '/bachida/user/sendMsgList?pageno='+ i))
		}
		if($pagination.next>0) {
			li = $("<li></li>").text('다음').appendTo(ul);
			li.wrapInner($("<a></a").attr('href', '/bachida/user/sendMsgList?pageno='+ $pagination.next));
		}	
	});
</script>
<style>
					...
					<div class="container">
						<div id="navbar" class="text-white collapse navbar-collapse">
							<ul class="nav navbar-nav">
								<li><a href="/bachida/user/receiveMsgList">받은쪽지함</a></li>
