...
<script>
	$(function() {
		...
		var $payment = $orderList.payment;
		console.log($order)
		console.log($orderList)
		
		var $total = 0;
		var $orderQuantity = 0;
		$.each($order,function(i,p){
			var $table = $("<table class='table table-condensed table-bordered table-order'></table>").appendTo("#orderList");
			var $thead = $("<thead></thead>").appendTo($table);
			var $thead_tr = $("<tr></tr>").appendTo($thead);
				$("<th></th>").addClass("text-center").text("작품정보").appendTo($thead_tr);
				$("<th width='10%'></th>").addClass("text-center").text("수량").appendTo($thead_tr);
				$("<th width='15%'></th>").addClass("text-center").text("가격").appendTo($thead_tr);
				$("<th width='25%'></th>").addClass("text-center").text("작가").appendTo($thead_tr);
			var $tbody = $("<tbody></tbody>").appendTo($table);
			var $tr = $("<tr></tr>").appendTo($tbody);
			var $td3 = $("<td></td>").addClass("pd20").text(p.productName).appendTo($tr);
			if (p.options!= null) {	// 옵션이 있으면
				for (var i = 0; i < p.options.length; i++) {
					$("<p></p>").text(p.options[i].optionContent + " : "+ p.options[i].optionQuantity).appendTo($td3);
					/* $("<input>").attr("type", "hidden").attr("name","options[" + i + "].optionContent").val(p.options[i].optionContent).appendTo("#frm");
					$("<input>").attr("type", "hidden").attr("name","options[" + i + "].cost").val(p.options[i].cost).appendTo("#frm");
					$("<input>").attr("type", "hidden").attr("name","options[" + i + "].optionQuantity").val(p.options[i].optionQuantity).appendTo("#frm");  */
				}
			}
			$("<td></td>").addClass("text-center").text(p.quantity).appendTo($tr);
			$("<td></td>").addClass("text-right").text(p.price).appendTo($tr);
			$td = $("<td></td>").addClass("text-center").appendTo($tr);
			$("<div></div>").text(p.artisanId).appendTo($td);
			...
			$("<td colspan='4'></td>").text("합계 " + p.price + " 원").addClass("text-right").appendTo($tr2);	
			$total = $total + p.price;
			$orderQuantity = $orderQuantity + p.quantity;
			$("<input>").val($orderQuantity).attr("name","orderQuantity").attr("type","hidden").appendTo("#frm");
			$("<input>").val(p.orderProductIdx).attr("name","cartProductIdx").attr("type","hidden").appendTo("#frm");// cartProductIdx값 넘기기
			
		})
		
		$("#orderName").val($order[0].orderName);
		$("#orderTel").val($order[0].orderTel);
		$("#orderEmail").val($order[0].orderEmail);
		$(".totalPrice").val($total);
		$("#cash").val($payment);
		if($payment<$total){
			$("#msg").text("캐쉬부족")
		}else{
			$("#msg").text("(결제 후 잔여캐쉬 : "+ Number($payment-$total) +" 원)")
		}
		$("#btn-order").on("click",function(){
			...
			$("#frm").attr("action","${pageContext.request.contextPath}/product/cartOrderWrite");
			$("#frm").serializeArray();
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<div class="text-right well">
						<h4>
							총 결제금액 <input name="totalPrice" readonly="readonly" class="input_text totalPrice"> 원
						</h4>
					</div>
				</div>
	...
	<div class="panel panel-default">
				...
				<div class="panel-body">
					<dl class="dl-horizontal form-inline">
						<dt>보유캐쉬</dt>
						<dd><input id="cash" type="text" readonly="readonly" class="input_text">원 <button type="button" class="btn btn-xs btn-default" data-toggle="modal" data-target="#charge">충전하기</button> <span id="msg" class="text-info"></span></dd>
						<dt>총 결제금액</dt>
					...
					<dl class="dl-horizontal form-inline">
						...
						<dd class="form-horizontal">
							...
							<input type="text" id="sample6_postcode" class="form-control" placeholder="우편번호" required>
								...
								<input type="text" id="sample6_address" class="form-control" placeholder="주소" required>
								<input type="text" id="sample6_address2" class="form-control" placeholder="상세주소" required>
						</dd>
					</dl>
				</div>
			</div>
			...
			<div class="text-center mgb20">
				<button class="btn btn-gray" type="button" onclick="goBack()">취소하기</button> &nbsp; <button class="btn btn-pink" type="button" id="btn-order">결제하기</button>
			</div>
		</form>
