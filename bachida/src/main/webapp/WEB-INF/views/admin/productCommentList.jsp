...
$(function(){
	...
	$.each($list, function(i, comment) {
		...
		$("<td></td>").text(comment.PRODUCTCOMMENTIDX).appendTo($tr);
		/* <td>
		<img class="pull-left" src="http://www.blueb.co.kr/SRC2/_image/w01.jpg" style="width: 100%; height:75px; max-width:150px;">
		<p>상품</p>
		<p>옵션명</p>
	</td> */
		var $imgTd = $("<td></td>").appendTo($tr);
		$("<img>").addClass("pull-left").attr("src","/bachida/user/displayFile?savedFileName="+comment.SAVEDFILENAME+"&originalFileName="+comment.ORIGINALFILENAME).css({"width":"200px","height":"175px"}).appendTo($imgTd);
		$("<p>").text(comment.PRODUCTNAME).appendTo($imgTd);
		$("<td></td>").text(comment.ID).appendTo($tr);
		$("<td></td>").text(comment.CONTENT).appendTo($tr);
		$("<td></td>").text(comment.WRITEDATE).appendTo($tr);
	});
	var ul = $("#paginationUl");
	var li;
...
<body>
	<div class="container">
		<div class="row">
			<h3 style="color: rgb(245, 89, 128);">상품평내역</h3>
			<div class="col-md-12">              
				<table class="table table-hover">
					<thead>
					<tr class="active">
						<th>번호</th><th>상품정보</th><th>구매자</th><th>평가내용</th><th>등록일</th>
					</tr>
					</thead>
					<tbody id="tbody">
					</tbody>
