...
<script>
	$(function(){
		...
		$.each($list, function(i, report) {
			...
			$("<td></td>").text(report.id).appendTo($tr);
			$("<td></td>").text(report.reportState).appendTo($tr);
		})
		$(".tdata").on("click",function(){
		var $idx = $(this).children().eq(0).text();
		console.log($idx);
		location.href = "/bachida/admin/reportView?reportIdx="+$idx;
	});
		var ul = $("#paginationUl");
		var li;
		if($pagination.prev>0) {
			li = $("<li></li>").text('이전').appendTo(ul);
			li.wrapInner($("<a></a").attr('href', '/bachida/admin/reportList?pageno='+ $pagination.prev))
		}
		for(var i=$pagination.startPage; i<=$pagination.endPage; i++) {
			li = $("<li></li>").text(i).appendTo(ul);
			if($pagination.pageno==i) 
				li.wrapInner($("<a></a").attr('href', '/bachida/admin/reportList?pageno='+ i).css({"background-color":"#337ab7", "border" : "1px solid #337ab7", "color":"white"}));
			else
				li.wrapInner($("<a></a").attr('href', '/bachida/admin/reportList?pageno='+ i))
		}
		if($pagination.next>0) {
			li = $("<li></li>").text('다음').appendTo(ul);
			li.wrapInner($("<a></a").attr('href', '/bachida/admin/reportList?pageno='+ $pagination.next));
		}	
	});
</script>
<style>
		...
		<div class="row">
			...
			<div class="col-md-12">
				<table class="table table-hover">
					<thead>
					<tr class="active">
						<th>신청번호</th><th>신고아이디</th><th>내용</th><th>신고일자</th><th>접수아이디</th><th>상태</th>
					</tr>
					</thead>
					<tbody id="tbody">
					</tbody>
